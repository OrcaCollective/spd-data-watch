...
def csv_to_dict(url: str) -> Dict[str, str]:
  """
  Creates a dict from a csv using the first column as the key and second column as the value.
  """
...
def find_case(case_num: str) -> CaseResult:
  rows = requests.get(f"https://data.seattle.gov/api/id/99yi-dthu.json?$query=select * where (upper(`file_number`) = upper('{case_num}'))").json()
  
  if not rows:
    return None
  
  officers = {validate(row["named_employee_id"], Regexps.SERIAL, "Unknown") for row in rows}
  allegations = {validate(row["allegation"], Regexps.STRING, "Unknown") for row in rows}
  disposition = {validate(row["disposition"], Regexps.STRING, "Unknown") for row in rows}
