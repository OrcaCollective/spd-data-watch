...
from app.models import db, Refresh, RefreshStatus, Update, UpdateType
...
class ClosedCaseSummaryUpdater(Updater):
  ...
  def process_case(self, case, update_dt) -> Update:
    ...
    # ]
    update = Update()
    update.case_num = validate(case["case"]["description"], Regexps.CASE_NUM, "Invalid case number")
    update.create_date = update_dt
    update.disposition = validate(case["disposition"], Regexps.CCS_DISPOSITION, "Unknown")
    update.event_date = parser.parse(validate(case["posted_date"], Regexps.TIMESTAMP, "1970-01-01T00:00:00"))
...
class NewComplaintUpdater(Updater):
  ...
  def process_complaint(self, complaint, update_dt) -> Update:
    ...
    #   ...
    update = Update()
    update.case_num = validate(complaint["opa_case_number"], Regexps.CASE_NUM, "Invalid case number")
    update.create_date = update_dt
    update.event_date = parser.parse(validate(complaint["task_creation_date"], Regexps.TIMESTAMP, "1970-01-01T00:00:00"))
    update.type = self.get_update_type()
...
class ClosedInvestigationUpdater(Updater):
  ...
  def process_case(self, case_num, rows, update_dt) -> Update:
    ...
    for row in rows:
      officers.add(find_serial(validate(row["named_employee_id"], Regexps.SERIAL, "Unknown")))
    ...
    update.create_date = update_dt
    update.event_date = parser.parse(validate(rows[0]["investigation_end_date"], Regexps.TIMESTAMP, None))
  ...
  def process(self, data, update_dt) -> List[Update]:
    ...
    key_by_case = lambda d: d["file_number"]
    data = sorted(data, key=key_by_case)
    cases = {k: list(v) for k, v in itertools.groupby(data, key=key_by_case)}
    return [self.process_case(case_num, rows, update_dt) for case_num, rows in cases.items()]
...
def do_update(last_refresh, now):
  """ Retrieves updates from each updater and saves updates to the database. """
  refresh = Refresh()
  ...
  try:
    for updater, update_attr in updaters:
      ...
      current_app.logger.debug(f"Found {len(updates)} updates for updater %s", type(updater))
...
def update():
  ...
  if last_refresh:
    if (
      last_refresh.status == RefreshStatus.COMPLETED and now - last_refresh.refresh_date > current_app.config["REFRESH_INTERVAL"]
      or last_refresh.status in [RefreshStatus.STARTED, RefreshStatus.FAILED] and now - last_refresh.refresh_date > current_app.config["RETRY_INTERVAL"]
    ):
      ...
      current_app.logger.debug("Last run at %s, skipping update.", last_refresh.refresh_date)
  else:
