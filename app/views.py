from flask import current_app, Blueprint, jsonify, make_response, render_template, request
import re
...
def updates():
  ...
  updates = (Update.query
             .order_by(Update.create_date.desc(), Update.event_date.desc())
             .paginate(page=page, per_page=current_app.config["ITEMS_PER_PAGE"]))
...
def updates_atom():
  updates = Update.query.order_by(Update.create_date.desc(), Update.id.desc()).all()  # TODO: trim to last 30 days?
  last_refresh = Refresh.last_refresh()
  
  resp = make_response(render_template("updates.xml", updates=updates, last_update=last_refresh.refresh_date))
