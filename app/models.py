...
class Update(db.Model):
  ...
  event_date = db.Column(db.DateTime, nullable=False)  # time event happened (posted date etc)

  type = db.Column(db.Enum(UpdateType), nullable=False)
  officers = db.Column(db.JSON, nullable=False)  # Normalize as JSON
  allegations = db.Column(db.JSON, nullable=True)  # New complaints may not always have allegations
...
class Refresh(db.Model):
  ...
  @staticmethod
  def last_refresh():
    return Refresh.query.order_by(Refresh.refresh_date.desc()).limit(1).one_or_none()
  
  @staticmethod
  def last_completed_refresh():
    return Refresh.query.filter_by(status=RefreshStatus.COMPLETED).order_by(Refresh.refresh_date.desc()).limit(1).one_or_none()
